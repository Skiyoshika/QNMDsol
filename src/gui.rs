...
impl Default for QnmdSolApp {
    fn default() -> Self {
        ...
        Self {
            ...
            record_label: Language::English.default_record_label().to_owned(),
...
impl QnmdSolApp {
    ...
    fn draw_controller(&self, ui: &mut egui::Ui) {
        ...
        let ls_dot = ls_c
            + Vec2::new(
                self.gamepad_visual.lx * 25.0,
                -self.gamepad_visual.ly * 25.0,
            );
        painter.circle_filled(ls_dot, 10.0, Color32::from_rgb(0, 255, 255)); // Cyan
        painter.text(
            ls_c + Vec2::new(0.0, 40.0),
        ...
        let rs_dot = rs_c
            + Vec2::new(
                self.gamepad_visual.rx * 25.0,
                -self.gamepad_visual.ry * 25.0,
            );
        painter.circle_filled(rs_dot, 10.0, Color32::from_rgb(255, 0, 255)); // Magenta
        painter.text(
            rs_c + Vec2::new(0.0, 40.0),
            egui::Align2::CENTER_TOP,
            "IJKL",
            egui::FontId::proportional(12.0),
            Color32::GRAY,
        ...
        // A (Down) -> Space
        draw_btn(
            btn_c + Vec2::new(0.0, spacing),
            self.gamepad_visual.a,
            "A",
            "Space",
            Color32::GREEN,
        );
        // B (Right) -> Z
        draw_btn(
            btn_c + Vec2::new(spacing, 0.0),
            self.gamepad_visual.b,
            "B",
            "Z",
            Color32::RED,
        );
    ...
    fn show_start_screen(&mut self, ctx: &egui::Context) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.vertical_centered(|ui| {
                ui.add_space(100.0);
                ui.heading(
                    egui::RichText::new("Welcome to QNMDsol world！")
                        .size(30.0)
                ...
                egui::Frame::none()
                    ...
                    .inner_margin(egui::style::Margin::symmetric(30.0, 20.0))
                    .show(ui, |ui| {
                        ui.vertical_centered(|ui| {
                            ui.label(
                                egui::RichText::new(self.text(UiText::StartSubtitle)).size(16.0),
                            );
                            ui.add_space(16.0);
                            ui.horizontal(|ui| {
                                let button_style =
                                    egui::Button::new(egui::RichText::new("中文").size(16.0))
                                ...
                                let button_style =
                                    egui::Button::new(egui::RichText::new("English").size(16.0))
                                        .min_size(Vec2::new(140.0, 42.0))
                                        .rounding(Rounding::same(12.0));
                ...
                match msg {
                    ...
                    BciMessage::DataPacket(data) => {
                        ...
                        for (i, val) in data.iter().enumerate().take(self.wave_buffers.len()) {
                            if i < self.wave_buffers.len() {
                                ...
                                self.wave_buffers[i]
                                    .push([self.time, *val * self.display_gain + offset]);
                                if self.wave_buffers[i].len() > 500 {
                                    self.wave_buffers[i].remove(0);
                                }
                            }
                        }
                    }
                    BciMessage::CalibrationResult(_, max) => {
                        ...
                        if self.calib_rest_max == 0.0 {
                            ...
                            self.log(&format!("Base: {:.1}", max));
                        } else {
                            self.calib_act_max = max;
                            self.log(&format!("Act: {:.1}", max));
        ...
        egui::SidePanel::left("L").min_width(300.0).show(ctx, |ui| {
            ...
            if ui.button(btn_txt).clicked() {
                if !self.is_connected {
                    self.tx_cmd
                        .send(GuiCommand::Connect(self.connection_mode))
                        .unwrap();
                } else {
            ...
            if self.is_recording {
                ui.label(
                    egui::RichText::new(self.text(UiText::Recording))
                        .color(Color32::RED)
                        .small(),
                );
                    ...
                    .show(ui, |plot_ui| {
                        ...
                        for (i, buf) in self.wave_buffers.iter().enumerate() {
                            if !buf.is_empty() {
                                ...
                                plot_ui.line(
                                    Line::new(PlotPoints::new(buf.clone()))
                                        .name(format!("Ch{}", i + 1))
                                        .color(*col),
                                );
                                if let Some((min, max)) =
                                    buf.iter().fold(None, |acc, p| match acc {
                                        None => Some((p[1], p[1])),
                        ...
                        if self.follow_latest && self.time > 0.0 {
                            ...
                            let y_max = if max_y.is_finite() {
                                ...
                                self.vertical_spacing * self.wave_buffers.len() as f64
                            };
...
impl Language {
    fn text(&self, key: UiText) -> &'static str {
        match (self, key) {
            (Language::English, UiText::Title) => "QNMDsol demo v0.1 - Chinese Update",
            ...
            (Language::English, UiText::Controller) => "CONTROLLER VISUALIZER",
            ...
            (Language::English, UiText::KeyHint) => "Try Keys: Space / Z / X / C",
            ...
            (Language::Chinese, UiText::Title) => "QNMDsol 演示 v0.1 · 汉化更新",
            ...
            (Language::Chinese, UiText::KeyHint) => "模拟模式按下 Space / Z / X / C 试试",
