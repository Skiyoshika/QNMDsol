...
* *注意：串口端口可在 QNMDsol 界面中直接选择（下拉框）。如果连接失败，请先在设备管理器确认实际端口号（例如 `COM3/COM4/...`），再回到软件里切换。*
...
3.  **核心驱动文件 (DLL):**
    为了防止程序启动/连接时闪退，运行目录（QNMDsol 可执行文件同级目录，或仓库根目录）**必须** 包含以下文件：
    * `BoardController.dll`
    * `DataHandler.dll`
    * `vJoyInterface.dll`
...
**重要提示：**
* SIM 模式下的键盘输入只在 **QNMDsol 窗口获得焦点**时生效（这是正常的）。
